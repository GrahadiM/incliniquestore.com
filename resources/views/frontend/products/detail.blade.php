...
@section('meta_description', $data->meta_description ?? $data->excerpt)
@section('meta_keywords', $data->meta_keywords)

@section('content')
<section class="py-16 bg-white">
    <div class="w-full lg:max-w-5xl mx-auto px-4">

        {{-- Breadcrumb --}}
        <nav class="text-sm text-gray-500 mb-6">
            <a href="{{ route('frontend.index') }}" class="hover:text-primary-orange">Home</a>
            <span class="mx-2">/</span>
            <a href="{{ route('frontend.product.index') }}" class="hover:text-primary-orange">Shop</a>
            <span class="mx-2">/</span>
        ...
        <div class="flex flex-wrap items-center text-sm text-gray-500 mb-8 gap-4">
            <div class="flex items-center space-x-2">
                ...
                <span>{{ $data->category?->name ?? 'Admin' }}</span>
            </div>
